...
class SayTest extends AnyFunSuite with Matchers {
  ...
  test("twenty-three") {
    Say.inEnglish(23) should be(Some("twenty-three"))
  }
  ...
  test("one million two thousand three hundred forty-five") {
    Say.inEnglish(1002345) should be(
      Some("one million two thousand three hundred forty-five")
    )
  }
  ...
  test("a big number") {
    Say.inEnglish(987654321123L) should be(
      Some(
