...
class AcronymTest extends AnyFunSuite with Matchers {
  ...
  test("punctuation without whitespace") {
    Acronym.abbreviate("Complementary metal-oxide semiconductor") should be(
      "CMOS"
    )
  }
  ...
  test("very long abbreviation") {
    ...
    ) should be("ROTFLSHTMDCOALM")
  }

  test("consecutive delimiters") {
    Acronym.abbreviate("Something - I made up from thin air") should be(
      "SIMUFTA"
    )
  }
